...
def test_source_ast_cache_position_stmt_and_expr():
    ...
    stmt = cache.source_pos_to_node(Pos(5, 14), ast.stmt)
    ...
    expr = cache.source_pos_to_node(Pos(4, 12), ast.expr)
...
def test_source_ast_cache_range_expr_and_stmt():
    ...
    expr = cache.source_range_to_node(Range(Pos(4, 12), Pos(4, 17)), ast.expr)
    assert isinstance(expr, ast.BinOp)
    expr_range = Range.from_ast_node(expr)
    assert expr_range is not None
    assert expr_range.of_string(code_stmt_if) == "i % 2"

    stmt = cache.source_range_to_node(Range(Pos(8, 4), Pos(8, 12)), ast.stmt)
...
def test_source_ast_cache_range_source_code_and_end_pos():
    ...
    expr = cache.source_range_to_node(Range(Pos(2, 11), Pos(2, 26)), ast.expr)
...
def test_source_ast_cache_if_let_stmt_and_expr():
    ...
    inner_stmt = cache.source_pos_to_node(Pos(3, 15), ast.stmt)
...
def test_source_ast_cache_let_else_stmt_and_expr():
    ...
    expr = cache.source_range_to_node(Range(Pos(5, 11), Pos(5, 16)), ast.expr)
    assert isinstance(expr, ast.BinOp)
    expr_range = Range.from_ast_node(expr)
    assert expr_range is not None
    assert expr_range.of_string(code_stmt_let_else) == "x + 1"

    else_stmt = cache.source_pos_to_node(Pos(3, 15), ast.stmt)
...
def test_source_ast_cache_innermost_outermost_and_enclosing_scope():
    cache = _build_cache(code_nested_func)

    field = cache.source_pos_to_node(Pos(4, 17), ast.expr)
    assert isinstance(field, ast.Name)

    inner_func = cache.innermost_enclosing(field, ast.FunctionDef)
    assert isinstance(inner_func, ast.FunctionDef)
    assert inner_func.name == "inner_func"

    outer_func = cache.outermost_enclosing(field, ast.FunctionDef)
    assert isinstance(outer_func, ast.FunctionDef)
    assert outer_func.name == "outer_func"

    scope = cache.enclosing_scope(field)
    assert isinstance(scope, ast.ClassDef)
    assert scope.name == "A"

    module_scope = cache.enclosing_scope(cache.module)
    assert isinstance(module_scope, ast.Module)
