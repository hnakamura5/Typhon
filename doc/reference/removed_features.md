...
Typhon removes several Python features to enforce cleaner code and stricter scoping.

## Removed Keywords

- `del`: Deletion of variables is not supported. Rely on scope exit for cleanup.
...
- **Wildcard Import**: `from module import *` is forbidden to prevent namespace pollution.
...
- **Dead Code**: Code after `return` or `raise` is strictly forbidden.
