...
Provide a default value when the left-hand side is `None`.
...
Call a function only when it is not `None`.
...
Access an index/key only when the collection is not `None`.
...
Optional pipe operator. See [Pipe Operator](pipe_operator.md).
